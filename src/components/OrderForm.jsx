...
import { usePrinters, useArticleLibrary } from '../hooks/useOrders'
...
const Input = ({ className = '', ...props }) => (
  <input
    className={`w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:border-brand-500 text-sm ${className}`}
    {...props}
  />
)
...
const Select = ({ children, ...props }) => (
  <select
    className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2.5 text-white focus:outline-none focus:border-brand-500 text-sm"
    {...props}
  >
    {children}
  </select>
...
export default function OrderForm({ order, onClose, onSaved }) {
  ...
  const { articles, refresh: refreshArticles } = useArticleLibrary()
  ...
  // Auto-fill from article library
  useEffect(() => {
    if (form.article_number.length >= 2) {
      const matches = articles.filter(a =>
        a.article_number.toLowerCase().includes(form.article_number.toLowerCase())
      )
  ...
  const applyArticle = (article) => {
    setForm(f => ({
      ...
      order_name: article.description || f.order_name,
      material: article.material || f.material,
      color: article.color || f.color,
      print_hours: article.default_print_hours || f.print_hours,
      print_minutes: article.default_print_minutes || f.print_minutes,
      notes: article.notes || f.notes,
    }))
    setShowSuggestions(false)
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        print_hours: Number(form.print_hours),
        print_minutes: Number(form.print_minutes),
        notes: form.notes,
        created_by: user.id,
      }
      ...
      let result
      if (order?.id) {
        result = await supabase.from('print_orders').update(payload).eq('id', order.id)
      } else {
        result = await supabase.from('print_orders').insert({ ...payload, status: 'new' })
      }

      if (result.error) throw result.error

      // Update article library
      if (form.article_number) {
        await supabase.from('article_library').upsert({
          article_number: form.article_number,
          description: form.order_name,
          material: form.material,
          color: form.color,
          default_print_hours: Number(form.print_hours),
          default_print_minutes: Number(form.print_minutes),
          notes: form.notes,
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-t-2xl sm:rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-800 sticky top-0 bg-slate-900 z-10">
          <h2 className="text-white font-semibold">
            {order?.id ? 'Order bewerken' : 'Nieuwe printorder'}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <Field label="Omschrijving / ordernaam">
            <Input
              ...
              required
              placeholder="bv. Beugel motorsysteem"
            />
          </Field>
          ...
          <Field label="Opmerkingen">
            <textarea
              ...
              rows={2}
              placeholder="Extra instructies, instellingen, aandachtspunten…"
              className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:border-brand-500 text-sm resize-none"
            />
          </Field>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={saving}
              className="flex-1 bg-brand-500 hover:bg-brand-600 disabled:opacity-50 text-white rounded-lg py-2.5 text-sm font-medium transition-colors">
