...
const NavItem = ({ to, icon: Icon, label, badge }) => (
  <NavLink to={to} className={({ isActive }) =>
    `flex flex-col items-center gap-0.5 px-2 py-2 rounded-xl transition-colors relative
    ...
    <div className="min-h-screen bg-black pb-20 sm:pb-0">
      ...
      <nav className="sm:hidden fixed bottom-0 inset-x-0 bg-black/95 backdrop-blur border-t border-slate-800 z-40 pb-safe">
        <div className="flex justify-around py-2">
          ...
          <button
            onClick={signOut}
            className="flex flex-col items-center gap-0.5 px-2 py-2 text-slate-400 hover:text-slate-300 transition-colors"
          >
            <LogOut size={20} />
